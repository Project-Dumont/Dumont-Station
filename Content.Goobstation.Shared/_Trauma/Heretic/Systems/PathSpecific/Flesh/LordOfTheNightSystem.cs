...
public sealed partial class LordOfTheNightSystem : EntitySystem
{
    ...
    private void OnHit(Entity<LordOfTheNightComponent> ent, ref MeleeHitEvent args)
    {
        ...
        foreach (var hit in args.HitEntities)
        {
            ...
            // TODO: organ groups
            // Dumont start
            var arm = _body.GetBodyChildrenOfType(hit, Content.Shared.Body.Part.BodyPartType.Arm, symmetry: ent.Comp.ArmLeft).FirstOrNull()
                      ?? _body.GetBodyChildrenOfType(hit, Content.Shared.Body.Part.BodyPartType.Arm, symmetry: ent.Comp.ArmRight).FirstOrNull();
            // Dumont end

            if (arm is { } armEnt && _part.GetParentPartOrNull(armEnt.Id) is { } parent)
                // Dumont start
                _wound.AmputateWoundable(parent, armEnt.Id);
                // Dumont end
        }
    }
}
