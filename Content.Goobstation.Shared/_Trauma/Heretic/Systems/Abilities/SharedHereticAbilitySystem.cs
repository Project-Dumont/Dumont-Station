...
public abstract partial class SharedHereticAbilitySystem : EntitySystem
{
        ...
        FixedPoint2? boneHeal)
    {
        ...
        {
            // Dumont start
            var bones = _body.GetBodyChildrenWithComponent<WoundableComponent>(uid)
                .SelectMany(part => part.Component.Bone.ContainedEntities)
                .Where(HasComp<BoneComponent>)
                .Select(bone => (Id: bone, Comp: Comp<BoneComponent>(bone)));
            // Dumont end

            foreach (var bone in bones)
            {
                if (boneHeal is { } heal)
                    _trauma.ApplyDamageToBone(bone.Id, heal, bone.Comp); // heal is negative
                else
                    _trauma.SetBoneIntegrity(bone.Id, bone.Comp.IntegrityCap, bone.Comp);
            }
        }
        ...
        // Dumont start
        EntityManager.System<Content.Shared._Goobstation.Wizard.SanguineStrike.SharedSanguineStrikeSystem>().LifeSteal(uid, FixedPoint2.Zero, uid.Comp1);
        // Dumont end
        ...
        var missing = blood.BloodMaxVolume - blood.BloodSolution.Value.Comp.Solution.Volume;
        if (bloodHeal == null)
        {
