...
// Dumont start
using RatvarianLanguageSystem = Content.Shared.Speech.EntitySystems.SharedRatvarianLanguageSystem;
...
public abstract partial class SharedMansusGraspSystem : EntitySystem
{
    public override void Initialize()
    {
        ...
        // Dumont start
        SubscribeLocalEvent<RustGraspComponent, AfterInteractEvent>(OnRustInteract, before: new[] { typeof(TouchSpellSystem), typeof(SharedCrayonSystem) });
        SubscribeLocalEvent<TagComponent, AfterInteractEvent>(OnAfterInteract, before: new[] { typeof(TouchSpellSystem), typeof(SharedCrayonSystem) });
        // Dumont end
    }
    ...
    private void OnTouchSpellUsed(Entity<MansusGraspComponent> ent, ref TouchSpellUsedEvent args)
    {
        ...
        _stamina.TakeStaminaDamage(target, ent.Comp.StaminaDamage, source: args.User, applyResistances: false);
