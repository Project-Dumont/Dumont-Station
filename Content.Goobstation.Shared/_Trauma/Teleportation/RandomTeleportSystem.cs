...
public sealed partial class RandomTeleportSystem : EntitySystem
{
    ...
    [SubscribeLocalEvent]
    private void OnUseInHand(Entity<HereticRandomTeleportOnUseComponent> ent, ref UseInHandEvent args)
    {
        if (args.Handled)
            return;
        ...
        {
            _stack.SetCount(ent.Owner, stack.Count - 1, stack);
            return;
        }
    ...
    public bool RandomTeleport(EntityUid target, Entity<HereticRandomTeleportComponent> rtp, bool sound = true, EntityUid? user = null, bool predicted = true)
        => RandomTeleport(target, rtp, out _, sound, user, predicted);

    public bool RandomTeleport(EntityUid target, Entity<HereticRandomTeleportComponent> rtp, out Vector2 finalWorldPos, bool sound = true, EntityUid? user = null, bool predicted = true)
    {
        ...
        _sparks.DoSparks(oldCoords); // different source entity from below so they use different rng seeds
