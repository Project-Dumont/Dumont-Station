...
public sealed partial class TackleSystem : EntitySystem
{
        ...
        float speed)
    {
        ...
        if (targetKnockdown > TimeSpan.Zero)
            _stun.TryKnockdown(target, targetKnockdown, drop: result > mod.DisarmThreshold * 1.5f);
        ...
        if (stamDamage > 0f)
            _stam.TakeStaminaDamage(target, stamDamage, source: user, applyResistances: false);
    ...
    private bool HandleTackle(ICommonSession? session, EntityCoordinates coords, EntityUid uid)
    {
        ...
        EnsureComp<TacklerComponent>(player);
    ...
    public bool TryTackle(Entity<TacklerComponent?, TransformComponent?> ent, EntityCoordinates coords)
    {
        ...
        if (staminaCost > 0f)
            _stam.TakeStaminaDamage(ent, staminaCost, applyResistances: false);
