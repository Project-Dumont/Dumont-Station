...
public sealed partial class GhoulSystem : SharedGhoulSystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<GhoulComponent, MapInitEvent>(OnGhoulInit, after: [typeof(Content.Shared.Body.Systems.SharedBodySystem)]);
        SubscribeLocalEvent<ShatteredRisenComponent, MapInitEvent>(OnRisenMapInit, after: [typeof(Content.Shared.Body.Systems.SharedBodySystem)]);
    }
    ...
    {
        if (ent.Comp.SkillEffectRemove is {} skillEffectRemove)
            _effect.TryApplyEffect(ent, skillEffectRemove, predicted: false);
    ...
    public void GhoulifyEntity(Entity<GhoulComponent> ent)
    {
        ...
        if (ent.Comp.SkillEffect is {} skillEffect)
            _effect.TryApplyEffect(ent, skillEffect, predicted: false);

        EntityManager.System<Content.Server._EinsteinEngines.Language.LanguageSystem>().AddLanguage(ent, "TauCetiBasic");
        ...
        else
        {
            ...
            if (!hasMind && TryComp(ent, out MindContainerComponent? mindContainer) &&
                TryComp(mindContainer.LastMindStored, out MindComponent? oldMindComp) &&
                oldMindComp.UserId != null &&
                !HasComp<MobStateComponent>(oldMindComp.OwnedEntity))
            {
                _mind.TransferTo(mindContainer.LastMindStored.Value, ent, mind: oldMindComp);
                _mind.UnVisit(mindContainer.LastMindStored.Value, oldMindComp);
                hasMind = true;
            }
            else
            {
        ...
        // Dumont start
        if (ent.Comp.ChangeHumanoidProfile && TryComp<HumanoidAppearanceComponent>(ent, out var humanoid))
        {
            ent.Comp.OldSkinColor = humanoid.SkinColor;
            ent.Comp.OldEyeColor = humanoid.EyeColor;

            var grey = Color.FromHex("#505050");
            humanoid.EyeColor = grey;
            _humanoid.SetSkinColor(ent, grey, verify: false, humanoid: humanoid);
            Dirty(ent, humanoid);
        }
        // Dumont end
    ...
    private void OnMobStateChange(Entity<GhoulComponent> ent, ref MobStateChangedEvent args)
    {
        ...
        foreach (var giblet in Body.GibBody(ent, ent.Comp.DeathBehavior == GhoulDeathBehavior.GibOrgans))
        {
