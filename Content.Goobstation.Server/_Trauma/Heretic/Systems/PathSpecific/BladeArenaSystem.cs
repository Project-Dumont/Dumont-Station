...
public sealed partial class BladeArenaSystem : SharedBladeArenaSystem
{
    ...
    private void OnMobStateChanged(Entity<HereticArenaParticipantComponent> ent, ref MobStateChangedEvent args)
    {
        ...
        if (TryComp(origin, out DamageableComponent? dmg))
            _lifesteal.LifeSteal(origin, victor.HereticHealPerCrit, dmg);

        // heal all wounds
        foreach (var (_, _, woundable) in _body.GetBodyChildrenWithComponent<WoundableComponent>(origin))
        {
        ...
        int tileRadius)
    {
        ...
        foreach (var tile in _tilesToConvert)
        {
            ...
            _tile.ReplaceTile(tile, replacement, grid, gridComp);
        }
