...
public sealed partial class ChatSystem : SharedChatSystem
{
    ...
    [Dependency] private readonly InteractionSystem _interactionSystem = default!; // Gaby Station -> Languages
    ...
    public void TrySendInGameICMessage(
        ...
        bool ignoreActionBlocker = false,
        string wrappedMessagePostfix = "", // Goobstation
        LanguagePrototype? languageOverride = null // Gaby Station -> Languages
        )
    {
        ...
        switch (desiredType)
        {
            case InGameICChatType.Speak:
                ...
                SendEntitySpeak(source, message, range, nameOverride, hideLog, ignoreActionBlocker, wrappedMessagePostfix, languageOverride: languageOverride);
                break;
            case InGameICChatType.Whisper:
                // Goob edit
                // Gaby Station -> Languages
                SendEntityWhisper(source, message, range, null, nameOverride, hideLog, ignoreActionBlocker, wrappedMessagePostfix, languageOverride: languageOverride);
                break;
        ...
        )
    {
        ...
        var wrappedMessage = WrapPublicMessage(source, name, message, wrappedMessagePostfix);
        ...
        var wrappedObfuscated = WrapPublicMessage(source, name, obfuscated, wrappedMessagePostfix);
        ...
        )
    {
        ...
        foreach (var (session, data) in GetRecipients(source, WhisperMuffledRange))
        {
            ...
            if (data.Range <= WhisperClearRange)
            {
                ...
                wrappedMessage = Loc.GetString("chat-manager-entity-whisper-wrap-message" + wrappedMessagePostfix,
                    ("entityName", name),
            ...
            else
            {
                ...
                result = ObfuscateMessageReadability(perceivedMessage);
                wrappedMessage = Loc.GetString("chat-manager-entity-whisper-unknown-wrap-message" + wrappedMessagePostfix,
                    ("message", FormattedMessage.EscapeText(result)));
            }
        ...
        var replayWrap = Loc.GetString("chat-manager-entity-whisper-wrap-message" + wrappedMessagePostfix,
            ("entityName", name),
    ...
    private void SendInVoiceRange(
        ...
        string message,
        string wrappedMessage,
        string obfuscated, // Gaby Station -> Languages
        string obfuscatedWrappedMessage, // Gaby Station -> Languages
        EntityUid source,
        ChatTransmitRange range,
        NetUserId? author = null,
        LanguagePrototype? languageOverride = null // Gaby Station -> Languages
    ...
    {
        var language = languageOverride ?? _language.GetLanguage(source); // Gaby Station -> Languages
    ...
    /// </summary>
    public string WrapPublicMessage(EntityUid source, string name, string message, string wrappedMessagePostfix = "")
    {
        var speech = GetSpeechVerb(source, message);
        var verbName = Loc.GetString(_random.Pick(speech.SpeechVerbStrings));
        return Loc.GetString((speech.Bold ? "chat-manager-entity-say-bold-wrap-message" : "chat-manager-entity-say-wrap-message") + wrappedMessagePostfix,
            ("entityName", name),
            ("verb", verbName),
    ...
    public string ObfuscateMessageReadability(
        string message,
        float chance = DefaultObfuscationFactor // Gaby Station -> Languages
        )
    {
...
public sealed class EntitySpokeEvent : EntityEventArgs
{
    ...
    public readonly bool IsWhisper; // Gaby Station -> Languages
    public readonly LanguagePrototype Language; // Gaby Station -> Languages
    ...
    public EntitySpokeEvent(EntityUid source, string message, RadioChannelPrototype? channel, bool isWhisper, LanguagePrototype language)
    {
        ...
        Message = message;
        Channel = channel;
        IsWhisper = isWhisper; // Gaby Station -> Languages
        Language = language; // Gaby Station -> Languages
    }
}
