...
public sealed class SecretDirectorSystem : GameRuleSystem<SecretDirectorComponent>
{
    ...
    protected override void Added(EntityUid uid, SecretDirectorComponent component, GameRuleComponent gameRule, GameRuleAddedEvent args)
    {
        base.Added(uid, component, gameRule, args);
        var weights = _configurationManager.GetCVar(CCVars_Funky.DirectorWeightPrototype);

        if (!TryPickPreset(weights, out var preset))
        {
            Log.Error($"{ToPrettyString(uid)} failed to pick any director preset. Removing rule.");
            Del(uid);
            return;
        }

        Log.Info($"Selected {preset.ID} as the director preset.");
        _adminLogger.Add(LogType.EventStarted, $"Selected {preset.ID} as the director preset.");
    ...
    protected override void Ended(EntityUid uid, SecretDirectorComponent component, GameRuleComponent gameRule, GameRuleEndedEvent args)
    {
