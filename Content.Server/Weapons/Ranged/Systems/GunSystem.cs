...
public sealed partial class GunSystem : SharedGunSystem
{
        ...
        EntityCoordinates fromCoordinates, EntityCoordinates toCoordinates, out bool userImpulse, EntityUid? user = null, bool throwItems = false)
    {
        ...
        var fromEnt = MapSystem.TryFindGridAt(fromMap, out var gridUid, out _)
    ...
    private Angle GetRecoilAngle(TimeSpan curTime, GunComponent component, Angle direction, EntityUid? user = null) // Goobstation user
    {
        ...
        // Dumont start
        var maxSpread = Math.Max(Math.Abs(minTheta), Math.Abs(maxTheta)) * Math.Abs(angleEv.Modifier);
        DebugTools.Assert(double.IsFinite(spread) && Math.Abs(spread) <= maxSpread);
        // Dumont end
        return angle;
    }
