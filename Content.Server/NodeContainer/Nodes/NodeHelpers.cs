...
namespace Content.Server.NodeContainer.Nodes
{
    ...
    {
        public static IEnumerable<Node> GetNodesInTile(EntityQuery<NodeContainerComponent> nodeQuery, EntityUid gridUid, MapGridComponent grid, SharedMapSystem mapSystem, Vector2i coords)
        {
        ...
        {
            foreach (var (dir, entityUid) in GetCardinalNeighborCells(gridUid, grid, mapSystem, coords, includeSameTile))
            {
                if (!nodeQuery.TryGetComponent(entityUid, out var container))
                    continue;
