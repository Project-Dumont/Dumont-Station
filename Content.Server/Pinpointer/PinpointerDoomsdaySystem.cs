...
public sealed class PinpointerDoomsdaySystem : EntitySystem
{
    ...
    {
        System.Console.WriteLine("[DEBUG_LOG] PinpointerDoomsdaySystem: Received MalfAiDoomsdayStartedEvent");

        // Track the AI brain specifically to find it in the event they're shunted
        var aiBrain = ev.Ai;

        System.Console.WriteLine($"[DEBUG_LOG] PinpointerDoomsdaySystem: Station: {ev.Station}, AI Brain: {aiBrain}");
    ...
    private void OnEntityTerminating(ref EntityTerminatingEvent ev)
    {
        ...
        {
            System.Console.WriteLine("[DEBUG_LOG] PinpointerDoomsdaySystem: AI brain deleted, reverting pinpointers");
    ...
    private void RevertPinpointersToNuclearDisk()
    {
        ...
        System.Console.WriteLine($"[DEBUG_LOG] PinpointerDoomsdaySystem: Found nuclear disk: {nukeDisk}");
        ...
        foreach (var pinpointerUid in _doomsdayTargetedPinpointers.ToList())
        {
            ...
            revertedCount++;
            System.Console.WriteLine($"[DEBUG_LOG] PinpointerDoomsdaySystem: Reverted pinpointer {pinpointerUid} back to nuclear disk");
        }

        System.Console.WriteLine($"[DEBUG_LOG] PinpointerDoomsdaySystem: Reverted {revertedCount} pinpointers back to nuclear disk tracking");
