...
public sealed partial class AlertsUI : UIWidget
{
        ...
        IReadOnlyDictionary<AlertKey, AlertState> alertStates)
    {
        foreach (var (alertKey, alertState) in alertStates)
        {
            ...
            {
                Sawmill.Warning($"found alertkey without alerttype, alert keys should never be stored without an alerttype set: {alertKey}");
                continue;
            }
            ...
            {
                Sawmill.Error($"Unrecognized alertType {alertType}");
                continue;
            }
